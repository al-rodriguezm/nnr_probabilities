...
png("./plots/ramachandran.png",width = 1024, height = 1024)
x <- seq(-180, 179)
y <- seq(180, -179)
Phi = c(-80, -60, -130)
Psi = c(-20, 145, 140)
...
for(ang in seq(length(Phi))){
  d=25
  tl <- c(Phi[ang]-d, Psi[ang]+d)
  tr <- c(Phi[ang]+d, Psi[ang]+d)
  bl <- c(Phi[ang]-d, Psi[ang]-d)
  br <- c(Phi[ang]+d, Psi[ang]-d)
...
text(-120, -20, labels=expression(alpha))
text(-10, 145, labels=expression(PPII))
text(-170, 140, labels=expression(beta))
dev.off()
```
...
png("./plots/nnr_probabilities.png",width = 1024, height = 1024)
x=seq(-180,179)
y=seq(180,-179)
...
for(a in arrays){
  ...
  # par(pin = c(3.32, 2.75))
  if( n==1 || n==4 ) {
  axis(side = 2, at=axposy, labels = axtexty, cex.axis = 0.70)
}
...
for(a in arrays2){
  ...
  axis(side = 1, at=axposx, labels = axtextx, cex.axis = 0.70)
  n=n+1
}
