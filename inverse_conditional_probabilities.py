...
probs=[]
probarray={}
array_aa1=np.load(str("arrays/array_aa" + str(len_aa) + ".npy"))  # Probabilidad individual del último aminoácido
...
for i in combinations:
    dictionary={}
    dictionary[i[0]+"aa"+str(len(probs))] = probs[0].get(str(i[0]))
    for j in range(1,len(probs)):
        if i[j-1]=="alpha":
            dictionary[i[j]+"aa"+str(len(probs)-j)]=probs[j][0].get(str(i[j]))
        if i[j-1]=="ppii":
            dictionary[i[j]+"aa"+str(len(probs)-j)]=probs[j][1].get(str(i[j]))
        if i[j-1]=="beta":
            dictionary[i[j]+"aa"+str(len(probs)-j)]=probs[j][2].get(str(i[j]))
        if i[j-1]=="remaining":
            dictionary[i[j]+"aa"+str(len(probs)-j)]=probs[j][3].get(str(i[j]))
...
for i in dictionaries:
    ...
    for j in reversed(range(0,len(i))):
        ...
        if keys[n]==str("alphaaa"+str(j+1)):
            peptide+="1"
        if keys[n]==str("ppiiaa"+str(j+1)):
            peptide+="2"
        if keys[n]==str("betaaa"+str(j+1)):
            peptide+="3"
        if keys[n]==str("remainingaa"+str(j+1)):
            peptide+="4"
        n+=1
    
    value=(peptide[::-1],total_probability)
...
with open('probabilities/inverse_conditional_probabilities.csv','w',newline='') as out:
